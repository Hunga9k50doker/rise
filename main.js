  ...
  async executeTransfers(wallet) {
    ...
    for (let i = 0; i < wallets.length; i++) {
      ...
      if (i < numberOfTransfers - 1) {
        ...
        await sleep(timeSleep * 1000); // Chuyển đổi giây sang mili giây
      }
    }
  ...
  async handleFaucet() {
    for (const syb of settings.TOKENS_FAUCET) {
      try {
        ...
        const urlCheck = `https://faucet-api.riselabs.xyz/faucet/multi-eligibility?address=${wallets.address}&tokens=${syb}`;
        const headers = {
        ...
        if (!responseCheck.data.ok || !responseCheck.data.results[token].eligible) {
